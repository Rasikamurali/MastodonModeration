# robustness_rule_deduplicate.py
#
# Robustness check: replicates the topic count distribution plot (Figure 2A)
# using deduplicated rules to verify that near-duplicate rules within instances
# do not inflate topic counts. Input is the LLM category encoding computed from
# the Jaccard-deduplicated rule set.
#
# Input:  data/deduplicated_oneshot.csv  (LLM category encoding on deduplicated rules)
# Output: figures/robustness_topic_count_deduplicated_rules.png/.pdf
...
violin_plot = "robustness_topic_count_deduplicated_rules"
os.makedirs("figures", exist_ok=True)
plt.savefig(f"figures/{violin_plot}.png", dpi=300)
plt.savefig(f"figures/{violin_plot}.pdf", dpi=300)
plt.show()
