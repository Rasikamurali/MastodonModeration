# figure_3_heatmap.py
#
# Heatmap of rule topic prevalence by instance size bin. For each bin (rows)
# and rule topic (columns), shows the fraction of instances that have at least
# one rule covering that topic. Darker green = more prevalent. Topics are
# ordered by the ABCD taxonomy (Action, Behavior, Content, Distribution).
#
# Input:  data/one_shot_llm_category_encoding.csv  (GPT rule topic categories per instance)
#         data/community_rules_data.csv             (user counts per instance)
# Output: figures/heatmap.png, figures/heatmap.pdf
# Run from project root: python code/analysis/main/figure_3_heatmap.py
...
# Count how many distinct topics appear in each instance (used as denominator for prevalence)
...
# Column order groups topics by broad category (ABCD taxonomy):
# Automated tools (Action) | Harassment/Doxxing/etc. (Behavior) | Hate Speech/NSFW/etc. (Content) | Spam/Ads/etc. (Distribution)
column_order = [
    "Automated tools",
    "Harassment", 
...
plt.savefig(f"figures/{heatmap}.png", dpi=300)
plt.savefig(f"figures/{heatmap}.pdf", dpi=300)
plt.tight_layout()
plt.show()
