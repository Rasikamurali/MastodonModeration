# figure4_ttr_bootstrapped.py
#
# Plots Type-Token Ratio (TTR) of instance rule sets by instance size bin.
# TTR = unique words / total words across all concatenated rules per instance.
# Higher TTR = more varied vocabulary. Rules are cleaned and joined before computing.
# Plotted with violin + scatter + bootstrapped mean (95% CI) per bin.
#
# Input:  data/community_rules_data.csv  (translated rules with user counts)
# Output: figures/ttr_bootstrapped.png, figures/ttr_bootstrapped.pdf
# Run from project root: python code/analysis/main/figure4_ttr_bootstrapped.py
...
# Concatenate all cleaned rules for each instance into a single string before computing TTR —
# TTR on a single short rule is unreliable; the full rule set gives a stable estimate
grouped_df = merged_df.groupby('Instance Name').agg(
    All_Rules=('cleaned translated text', lambda x: ' '.join(x)),  # Concatenate rules
    Total_User_Count=('User Count', 'first'),
...
plt.savefig(f"figures/{ttr_overlay_plot}.png", dpi=300)
plt.savefig(f"figures/{ttr_overlay_plot}.pdf", dpi=300)
plt.show()
