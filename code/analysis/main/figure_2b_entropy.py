# figure_2b_entropy.py
#
# Measures how evenly an instance's rules are spread across four broad topic
# categories (Action, Behavior, Content, Distribution) using Shannon entropy.
# Higher entropy = more diverse rule set. Range: 0 (single topic) to ~2 (all equal).
# Plots entropy vs. instance size bin with violin + scatter + bootstrapped mean (95% CI).
#
# Input:  data/one_shot_llm_category_encoding.csv  (GPT rule topic categories per instance)
# Output: figures/entropy_plot.png, figures/entropy_plot.pdf
# Run from project root: python code/analysis/main/figure_2b_entropy.py
...
# Maps the 20 fine-grained GPT rule topics into 4 broad categories for entropy calculation:
#   Action (automated tools), Behavior (harassment, doxxing, etc.),
#   Content (hate speech, NSFW, etc.), Distribution (spam, advertising, etc.)
taxonomy = {
...
# Shannon entropy: H = -sum(p * log2(p)) over non-zero category proportions.
# Max entropy = log2(4) ≈ 2 when all four categories are equally represented.
# Entropy = 0 when all rules fall into a single category.
...
plt.savefig(f"figures/{topicdiv_overlay_plot}.png", dpi=300)
plt.savefig(f"figures/{topicdiv_overlay_plot}.pdf", dpi=300)
plt.show()
