...
Automates the collection of Mastodon instance metadata and rules from the instances.social API.
- Fetches instances in user count bins (1–5, 6–15, ..., 5001+).
- Saves each group as a CSV.
- Concatenates all groups into a single dataset.
- Performs stratified sampling for analysis.
- Fetches and saves rules for each sampled instance.

Outputs:
- Instances_<min>_<max>.csv: Raw instance data for each user count bin.
- Complete_dataset.csv: All instances combined.
- final_dataset.csv: Stratified sample of instances.
- instance_count.csv: Rule counts per instance.
- instance_rules.csv: Cleaned rules per instance.
"""
import pandas as pd 
import numpy as np 
import os 
...
load_dotenv()
id = os.getenv('INSTANCES_API_ID')
id_token = os.getenv('INSTANCES_API_TOKEN')
